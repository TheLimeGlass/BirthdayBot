		...
		for (IRole role : roles) {
			if (author.hasRole(role) || event.getGuild().getOwner().equals(author)) {
				switch (update(author)) {
					case 1:
						scheduledMessage(event.getChannel(), 60, "Setup the channel that you would like the Birthdays to be broadcasted in. Type **:tada: setup #channel-mention**");
						return;
					case 2:
						...
						if (!message.getChannelMentions().isEmpty()) {
							...
							message(event.getChannel(), "The main birthdays channel has been set to " + channel.mention() + ".");
